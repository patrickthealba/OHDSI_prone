...
configuration { /* All configuration for this annotator. */
    ...
    "time" {
        expressions = [
                ...
                '\\d\\d?:\\d\\d(\\s*am|\\s*pm)?', //1:00, 12:00
                '\\d\\d(\\s*am|\\s*pm)'

        ]
        outputType = "gov.va.vinci.vent.types.Temporality"
        concept_feature_value = "Treated:time"
    }
    "proximal_time" {
        expressions = [
                ...
                'tonight',
                'overnight',
                '(in\\s*the|this)\\s*morning',
                '(in\\s*the|this)\\s*AM',
                '(in\\s*the|this)\\s*afternoon',
                'last\\s*night',
                'monday',     //abbreviated days were too imprecise
                'tuesday',
                'wednesday',
                'thursday',
                'friday',
        ...
        outputType = "gov.va.vinci.vent.types.Temporality"
        concept_feature_value = "Treated:proximal_time"
    }
    "duration" {
        expressions = [

                'for\\s*(one|two|three|four|five|six|seven|eight|nine|ten)\\s*(seconds?|minutes?|hours?|days?|weeks?)',
                'for\\s*\\d\\d?\\s*(seconds?|minutes?|hours?|days?|weeks?)',
                '16/8\\s*hours' //Prone specific
